         ...
         std::string &v)
{
  ...
  // acc->commit(...), and return false.
	...
	for (int i = 0; i < nodes.size(); i ++) {
		std::string cid = nodes.at(i);
		handle h(cid);
		rpcc *cl = h.safebind();
		if(cl){
			paxos_protocol::prepareres r;
			paxos_protocol::status ret = cl->call(paxos_protocol::preparereq, me, a, r, rpcc::to(1000));
        ...
        std::vector<std::string> nodes, std::string v)
{
  paxos_protocol::acceptarg a = {instance, my_n, v};
	for (int i = 0; i < nodes.size(); i ++) {
		...
		if (cl){
			bool r = false;
			paxos_protocol::status ret = cl->call(paxos_protocol::acceptreq, me, a, r, rpcc::to(1000));
			if (r && ret == paxos_protocol::OK) {
				accepts.push_back(cid);
			}
		} else {
			printf("accept bind fail\n");
		}
	}
}
	      ...
	      std::string v)
{
  // You fill this in for Lab 6
	...
	for (int i = 0; i < accepts.size(); i ++) {
		std::string cid = accepts.at(i);
		handle h(cid);
		rpcc *cl = h.safebind();
		if(cl){
			int r;
			paxos_protocol::status ret = cl->call(paxos_protocol::decidereq, me, a, r, rpcc::to(1000));
		} else {
