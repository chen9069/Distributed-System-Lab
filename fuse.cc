                   ...
                   int to_set, struct fuse_file_info *fi)
{
  ...
  if (FUSE_SET_ATTR_SIZE & to_set) {
    ...
    yfs_client::inum inum = ino; // req->in.h.nodeid;
		...
		if (yfs->setsize(inum, size) != yfs_client::OK) {
			fuse_reply_err(req, ENOENT);
			return;
		}
    yfs_client::status ret;
...
fuseserver_write(fuse_req_t req, fuse_ino_t ino,
                 ...
                 struct fuse_file_info *fi)
{
printf("write file %016llx\n", ino);
  // You fill this in for Lab 2
#if 1
  // Change the above line to "#if 1", and your code goes here
	if (yfs->write(ino, size, off, buf) != yfs_client::OK) {
		fuse_reply_write(req, ENOENT);
	}
	else
                        ...
                        mode_t mode, struct fuse_entry_param *e)
{
  ...
  yfs_client::status ret = yfs->createfile(parent, true, name, inum, getpid());
  if(ret != yfs_client::OK){
		// create file fail;
  	return ret;
  }
...
yfs_client::status
fuseserver_createdirhelper(fuse_ino_t parent, const char *name,
                        mode_t mode, struct fuse_entry_param *e)
{
  // In yfs, timeouts are always set to 0.0, and generations are always set to 0
	//printf("createhelper(%016llx, ...)\n", parent);
  ...
  yfs_client::status ret = yfs->createfile(parent, false, name, inum, getpid());
  if(ret != yfs_client::OK){
		// create file fail;
  	return ret;
  }
...
printf("lookup name %s\n", name);
  ...
  bool found = false;
	yfs_client::inum inum;
	if (yfs->lookupfile(parent, name, inum) == yfs_client::EXIST) {
		found = true;
		struct stat st;
  	if(getattr(inum, st) == yfs_client::OK){
			// no such file
...
printf("readdir %016llx\n", ino);
  ...
  // You fill this in for Lab 2
	std::string buf;
	if (yfs->getcontent(inum, buf) != yfs_client::OK) {
		fuse_reply_err(req, ENOENT);
		return;
	}
	...
	while (ist >> file) {
		...
		dirbuf_add(&b, fname.c_str(), finum);
	}
...
#if 1
  yfs_client::status ret;
  if( (ret = fuseserver_createdirhelper( parent, name, mode, &e )) == yfs_client::OK ) {
    fuse_reply_entry(req, &e);
  } else {
		if (ret == yfs_client::EXIST) {
			fuse_reply_err(req, EEXIST);
