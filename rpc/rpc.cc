		...
		unsigned int xid_rep, char **b, int *sz)
{
	...
	//printf("\nclt: %d, xid: %d, xid_rep: %d, window_size: %d\n", clt_nonce, xid, xid_rep, reply_window_[clt_nonce].size());
...
//printf("%d ", it->xid);
	...
	if (reply_window_[clt_nonce].size() == 0 || reply_window_[clt_nonce].back().xid < xid) {
		//printf("last xid: %d\n", reply_window_[clt_nonce].back().xid);	
		reply_t rep(xid);		
		reply_window_[clt_nonce].push_back(rep);		
		return NEW;
	}
	else if (reply_window_[clt_nonce].front().xid > xid) {
		//printf("FORGOTTEN: %d\n", reply_window_[clt_nonce].front().xid);
		return FORGOTTEN;
	}
	else {
		...
		char *b, int sz)
{
	...
	if (it == reply_window_[clt_nonce].end()) {
		...
		while (iter != reply_window_[clt_nonce].end()) {
			printf("%d, %s, %d", iter->xid, iter->buf, iter->sz);
			iter ++;
		}
		printf("\n");
	}
