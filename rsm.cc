...
rsm::recovery()
{
  ...
  while (1) {
    ...
    if (primary == cfg->myaddr()) {
			tprintf("primary recovery: %s\n", primary.c_str());
      r = sync_with_backups();
    } else {
			tprintf("slave recovery\n");
      r = sync_with_primary();
    }
    tprintf("recovery: sync done %d %d %d\n", vid_insync, vid_commit, inviewchange);
...
rsm::statetransferdone(std::string m) {
  ...
  // - Inform primary that this slave has synchronized for vid_insync
	...
	int r;
  VERIFY(pthread_mutex_unlock(&rsm_mutex)==0);
	rsm_protocol::status ret = cl->call(rsm_protocol::transferdonereq, cfg->myaddr(), vid_insync, r, rpcc::to(1000));
  VERIFY(pthread_mutex_lock(&rsm_mutex)==0);
...
rsm::client_invoke(int procno, std::string req, std::string &r)
{
  ...
  // You fill this in for Lab 7
	if (inviewchange) {
		tprintf("inviewchange\n");
		return rsm_client_protocol::BUSY;
	}
	if (!amiprimary()) {
		return rsm_client_protocol::NOTPRIMARY;
	}
	std::vector<std::string> values = cfg->get_view(vid_commit);
	for (int i = 0; i < values.size(); i ++) {
		std::string t = values.at(i);
		if (t != cfg->myaddr()) {
			handle h(t);
			rpcc *cl = h.safebind();
			int dummy;
//pthread_mutex_unlock(&invoke_mutex);
			rsm_protocol::status rt = cl->call(rsm_protocol::invoke, procno, myvs, req, dummy, rpcc::to(1000));
breakpoint1();
partition1();
//pthread_mutex_lock(&invoke_mutex);
			if (rt != rsm_protocol::OK) {
				return rsm_client_protocol::BUSY;
			}
		}
	}
...
rsm_protocol::transferres &r)
{
  ...
  if (!insync || vid != vid_insync) {
		tprintf("transferrep busy %d %d %d\n", insync, vid, vid_insync);
